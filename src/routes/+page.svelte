<script>
	...
	import FHEPrintModal from "../lib/components/FHEPrintModal.svelte";
...
<div class="mx-auto p-2 sm:p-4 bg-gray-100 shadow-md rounded-lg" style="max-width:900px;">
	{#if serverMessage}
		<div class="mb-4 p-3 rounded border text-center" class:bg-green-100={serverMessageType === "success"} class:text-green-800={serverMessageType === "success"} class:border-green-300={serverMessageType === "success"} class:bg-red-100={serverMessageType === "error"} class:text-red-800={serverMessageType === "error"} class:border-red-300={serverMessageType === "error"}>
			{serverMessage}
		</div>
	{/if}
		...
		<fieldset class="border border-gray-600 p-2 sm:p-16 rounded-md">
			...
			<div class="mb-4 flex flex-col md:flex-row gap-2 md:gap-4 flex-wrap">
				...
				<div class="flex-1 min-w-0">
					...
					<input id="fullname" type="text" bind:value={fullname} class="mt-1 block w-full max-w-xs border-gray-700 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm" required />
				</div>
				<div class="flex-1 min-w-0">
			...
			<div class="mb-4 flex flex-col md:flex-row gap-2 md:gap-4 flex-wrap">
				...
				<div class="flex-1 min-w-0">
					...
					<input id="email" type="email" bind:value={email} class="mt-1 block w-full max-w-xs border-gray-700 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm" required />
				</div>
			</div>
			...
			<div class="mb-4 flex flex-col md:flex-row gap-2 md:gap-4 flex-wrap">
				...
				<div class="flex-1 min-w-0">
					...
					<select id="year_level" bind:value={year_level} class="mt-1 block w-full max-w-xs border-gray-700 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm" required>
			...
			<div class="mb-4 flex flex-col md:flex-row gap-2 md:gap-4 flex-wrap">
				<div class="flex-1 min-w-0">
					...
					<div class="flex flex-row gap-2 md:gap-6">
						...
						<label class="inline-flex items-center">
							<input id="scholarship_no" type="radio" bind:group={scholarship} value="No" class="form-radio h-5 w-5 text-blue-600" required />
					...
					{#if scholarship === "Yes"}
						<div class="mt-2">
							...
							<input id="scholarship_name" type="text" bind:value={scholarship_name} class="mt-1 block w-full max-w-xs border-gray-700 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm" required />
						</div>
					{/if}
				</div>
			</div>
			...
			<div class="mb-4 flex flex-col md:flex-row gap-2 md:gap-4 flex-wrap">
				<div class="flex-1 min-w-0">
					...
					<input id="father_name" type="text" bind:value={father_name} class="mt-1 block w-full max-w-xs border-gray-700 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm" required />
				</div>
				<div class="flex-1 min-w-0">
					<label for="mother_name" class="block text-sm font-medium text-gray-700 mb-1">Mother Name</label>
					<input id="mother_name" type="text" bind:value={mother_name} class="mt-1 block w-full max-w-xs border-gray-700 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm" required />
				</div>
			</div>
			<div class="mb-4 flex flex-col md:flex-row gap-2 md:gap-4 flex-wrap">
				<div class="flex-1 min-w-0">
					<label for="father_occupation" class="block text-sm font-medium text-gray-700 mb-1">Father Occupation</label>
					<input id="father_occupation" type="text" bind:value={father_occupation} class="mt-1 block w-full max-w-xs border-gray-700 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm" required />
				</div>
				<div class="flex-1 min-w-0">
					<label for="mother_occupation" class="block text-sm font-medium text-gray-700 mb-1">Mother Occupation</label>
					<input id="mother_occupation" type="text" bind:value={mother_occupation} class="mt-1 block w-full max-w-xs border-gray-700 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm" required />
				</div>
			</div>
			<div class="mb-4 flex flex-col md:flex-row gap-2 md:gap-4 flex-wrap">
				<div class="flex-1 min-w-0">
					<label for="father_income" class="block text-sm font-medium text-gray-700 mb-1">Father Estimated Income</label>
					<input id="father_income" type="text" bind:value={father_income} class="mt-1 block w-full max-w-xs border-gray-700 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm" required />
				</div>
				<div class="flex-1 min-w-0">
					<label for="mother_income" class="block text-sm font-medium text-gray-700 mb-1">Mother Estimated Income</label>
					<input id="mother_income" type="number" bind:value={mother_income} class="mt-1 block w-full max-w-xs border-gray-700 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm" required />
				</div>
			</div>
			...
			<div class="mb-4">
				...
				{#if studentsignatureData}
					<img src={studentsignatureData} alt="Student Signature Preview" style="max-width:200px;display:block;margin-bottom:8px;" />
					<button type="button" on:click={openStudentSignatureModal} class="px-3 py-1 bg-green-600 text-white rounded hover:bg-green-700 transition">Edit Signature</button>
				{:else}
					<button type="button" on:click={openStudentSignatureModal} class="px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 transition">Add Signature</button>
				{/if}
			</div>
			<SignatureModal show={showStudentSignatureModal} value={studentsignatureData} label="Student Signature" on:close={closeStudentSignatureModal} on:save={handleStudentSignatureSave} />
			...
			<div class="mb-4 flex flex-col md:flex-row gap-2 md:gap-4 flex-wrap">
				<div class="flex-1 min-w-0">
					...
					<input id="guardian_name" type="text" bind:value={guardian_name} class="mt-1 block w-full max-w-xs border-gray-700 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm" required />
				</div>
				<div class="flex-1 min-w-0">
					<label for="guardian_relationship" class="block text-sm font-medium text-gray-700 mb-1">Relationship to Student</label>
					<select id="guardian_relationship" bind:value={guardian_relationship} class="mt-1 block w-full max-w-xs border-gray-700 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm" required>
			...
			<div class="mb-4">
				...
				{#if guardianSignatureData}
					<img src={guardianSignatureData} alt="Guardian Signature Preview" style="max-width:200px;display:block;margin-bottom:8px;" />
					<button type="button" on:click={openGuardianSignatureModal} class="px-3 py-1 bg-green-600 text-white rounded hover:bg-green-700 transition">Edit Signature</button>
				{:else}
					<button type="button" on:click={openGuardianSignatureModal} class="px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 transition">Add Signature</button>
				{/if}
			</div>
			<SignatureModal show={showGuardianSignatureModal} value={guardianSignatureData} label="Guardian/Parent Signature" on:close={closeGuardianSignatureModal} on:save={handleGuardianSignatureSave} />
			...
			<div class="mb-4">
				...
				<label class="inline-block cursor-pointer bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition">
					Choose Picture
					<input id="student_picture" type="file" accept="image/*" on:change={handlePictureChange} class="hidden" required />
				</label>
			...
			<div class="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-10">
				...
				<button type="button" on:click={openPreviewModal} class="inline-block bg-gray-600 text-white px-4 py-2 rounded hover:bg-gray-700 transition" style="min-width: 140px;">Preview Form</button>
			</div>
		</fieldset>
	</form>
</div>
